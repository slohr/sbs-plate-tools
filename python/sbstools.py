...
class PlateMapUtil:
    ...
    def location_index_lookup_by_well(self, plate_type, well):
        ...
        column = int(well[1:])
        if plate_type == PlateMapUtil.PLATE_TYPE_96:
            return (ord(row) - PlateMapUtil.ASCII_CHAR_A_VALUE) * \
                PlateMapUtil.COLUMN_COUNT_96 + column
        elif plate_type == PlateMapUtil.PLATE_TYPE_384:
            return (ord(row) - PlateMapUtil.ASCII_CHAR_A_VALUE) * \
                PlateMapUtil.COLUMN_COUNT_384 + column
        else:
            raise ValueError(
                "Acceptable plate types: {0} or {1}".format(
                    PlateMapUtil.PLATE_TYPE_96,
    ...
    def translate_row_index_to_letter(self, row_index):
        if not row_index or row_index < 1 or row_index > PlateMapUtil.GLOBAL_MAX_ROW_INDEX:
            raise IndexError(
                "Index supplied does not map to any plate map configured: {0}".format(row_index))
    ...
    def translate_letter_to_row_index(self, letter):
        ...
        if row_index > PlateMapUtil.GLOBAL_MAX_ROW_INDEX:
            raise IndexError(
                "Letter supplied does not map to any plate map configured: {0}".format(letter))

        return row_index

    def location_96_to_location_384(self, quadrant, location_index):

        row96 = self.row_number_lookup_by_index(
            PlateMapUtil.PLATE_TYPE_96, location_index)
        column96 = self.column_number_lookup_by_index(
            PlateMapUtil.PLATE_TYPE_96, location_index)

        quadrant_one_location_index = (
            ((row96 * 2) - 2) * PlateMapUtil.COLUMN_COUNT_384) + ((column96 * 2) - 1)
    ...
    def location_384_to_location_96(self, location_index):
        row384 = self.row_number_lookup_by_index(
            PlateMapUtil.PLATE_TYPE_384, location_index)
        column384 = self.column_number_lookup_by_index(
            PlateMapUtil.PLATE_TYPE_384, location_index)
        ...
        return ((row96 * PlateMapUtil.COLUMN_COUNT_96) -
                PlateMapUtil.COLUMN_COUNT_96) + column96
