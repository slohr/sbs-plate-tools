...
class TestPlateMapUtil(unittest.TestCase):
    ...
    def test_WellLookupInputIndexLowerException96(self):
        self.assertRaises(
            IndexError,
            self.plate.well_lookup_by_index,
            PlateMapUtil.PLATE_TYPE_96,
            0)
    ...
    def test_LocationLookup96(self):
        res = self.plate.location_index_lookup_by_well(
            PlateMapUtil.PLATE_TYPE_96, 'A12')
        self.assertEquals(12, res)
        res = self.plate.location_index_lookup_by_well(
            PlateMapUtil.PLATE_TYPE_96, 'B01')
        self.assertEquals(13, res)
        res = self.plate.location_index_lookup_by_well(
            PlateMapUtil.PLATE_TYPE_96, 'B12')
        self.assertEquals(24, res)

    def test_LocationLookup384(self):
        res = self.plate.location_index_lookup_by_well(
            PlateMapUtil.PLATE_TYPE_384, 'B01')
        self.assertEquals(25, res)
        res = self.plate.location_index_lookup_by_well(
            PlateMapUtil.PLATE_TYPE_384, 'B24')
    ...
    def test_PlateTypeException(self):
        ...
        self.assertRaises(
            ValueError,
            self.plate.location_index_lookup_by_well,
            'blue',
            'B24')

    def test_RowLookupByIndex96(self):
        res = self.plate.row_number_lookup_by_index(
            PlateMapUtil.PLATE_TYPE_96, 1)
        self.assertEquals(1, res)
        res = self.plate.row_number_lookup_by_index(
            PlateMapUtil.PLATE_TYPE_96, 54)
        self.assertEquals(5, res)
        res = self.plate.row_number_lookup_by_index(
            PlateMapUtil.PLATE_TYPE_96, 96)
        self.assertEquals(8, res)

    def test_RowLookupByIndex384(self):
        res = self.plate.row_number_lookup_by_index(
            PlateMapUtil.PLATE_TYPE_384, 1)
        self.assertEquals(1, res)
        res = self.plate.row_number_lookup_by_index(
            PlateMapUtil.PLATE_TYPE_384, 203)
        self.assertEquals(9, res)
        res = self.plate.row_number_lookup_by_index(
            PlateMapUtil.PLATE_TYPE_384, 384)
    ...
    def test_TranslateLetterToRowIndexBoundaryException(self):
        self.assertRaises(
            IndexError,
            self.plate.translate_letter_to_row_index,
            'Q')
    ...
    def test_TranslateRowIndexToLetterBoundaryException(self):
        self.assertRaises(
            IndexError,
            self.plate.translate_row_index_to_letter,
            17)
    ...
    def test_ColumnNumberLookupByIndex96(self):
        res = self.plate.column_number_lookup_by_index(
            PlateMapUtil.PLATE_TYPE_96, 1)
        self.assertEquals(1, res)
        res = self.plate.column_number_lookup_by_index(
            PlateMapUtil.PLATE_TYPE_96, 58)
    ...
    def test_ColumnNumberLookupByIndex384(self):
        ...
        self.assertEquals(1, res)
        res = self.plate.column_number_lookup_by_index(
            PlateMapUtil.PLATE_TYPE_384, 214)
    ...
    def test_Location96ToLocation384(self):
        res = self.plate.location_96_to_location_384(
            PlateMapUtil.QUADRANT_ONE, 42)
        self.assertEquals(155, res)
        res = self.plate.location_96_to_location_384(
            PlateMapUtil.QUADRANT_TWO, 42)
        self.assertEquals(156, res)
        res = self.plate.location_96_to_location_384(
            PlateMapUtil.QUADRANT_THREE, 42)
        self.assertEquals(179, res)
        res = self.plate.location_96_to_location_384(
            PlateMapUtil.QUADRANT_FOUR, 42)
        self.assertEquals(180, res)

    def test_Location96ToLocation384BadQuadrantException(self):
        self.assertRaises(
            ValueError,
            self.plate.location_96_to_location_384,
            'Q5',
            42)
